...
impl BackupManager {
    ...
    pub async fn create_backup(&self, options: BackupOptions) -> Result<BackupResult> {
        ...
        let result = self
            .perform_backup(conn, &backup_path, &backup_id, &options)
            .await;
    ...
    async fn perform_backup(
        ...
        let dest_conn = SqliteConnection::connect(&format!("sqlite:{}", backup_path.display()))
            .await
            .map_err(|e| {
                DatabaseError::Sqlite(format!("Failed to create destination database: {}", e))
            })?;
